...
def get_document_comments(docxFileName):
    ...
    for comment_id in comments_dict:
        combined_dict[comment_id] = (comments_of_dict.get(comment_id, ''), comments_dict[comment_id])
...
if file_format == 'docx':
    ...
    # Setup character dictionary, scene heading list and current_scene_id for project.rdf
    characters = {}
    scene_headings = []
    current_scene_id = ''

    # Parse the .docx file
    comments_dict = get_document_comments(input_path)
    ...
    for paragraph in doc.paragraphs:
        ...
        # Handle comments
        comments = []
        for comment_id, (comment_source, comment_text) in list(comments_dict.items()):
            if comment_source in text:
                comment_date = datetime.now().strftime('%A, %d %B %Y %H:%M:%S')
                comments.append((comment_id, comment_text, comment_date))
        ...
        if '.' in text:
            ...
            if before_dot.isupper() and before_dot:
                ...
                html_body += f'<p scenestr="{scene_count}" scenenumber="{scene_count}" id="{scene_id}" class="sceneheading">{text}'
                for comment_id, comment_text, comment_date in comments:
                    html_body += f'<span date="{comment_date}" id="{comment_id}" text="{comment_text}" class="note"></span>'
                html_body += '<br>\n</p>\n'
        ...
        html_body += f'<p class="{element_class}">{text}'
        for comment_id, comment_text, comment_date in comments:
            html_body += f'<span date="{comment_date}" id="{comment_id}" text="{comment_text}" class="note"></span>'
        html_body += '<br>\n</p>\n'
    ...
    def apply_formatting(paragraph, class_name, comments):
        ...
        # Add comments
        for comment_id, comment_text, comment_date in comments:
            run.add_comment(comment_text, author="Author", initials="AU")
    ...
    for p in soup.find_all("p"):
        ...
        comments = []
        for span in p.find_all("span", class_="note"):
            comment_id = span.get("id")
            comment_text = span.get("text")
            comment_date = span.get("date")
            comments.append((comment_id, comment_text, comment_date))
        if class_name and text:
            paragraph = doc.add_paragraph(text)
            apply_formatting(paragraph, class_name, comments)
